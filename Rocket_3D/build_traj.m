function [states, inputs] = build_traj(t, x0, inputs)
  ...
  function f = get_f(x_c, x_f, u_c, u_f, t_c, t_f)
  %   f = x_i+1 - x_i - 0.5*h*(y_i+1 + y_i) = 0
    h = t_f - t_c;
    f = x_f - x_c - 0.5*h*(diffeq(x_f,u_f,t_f) + diffeq(x_c,u_c,t_c));
  end
  ...
  for i = 2:N
    ...
    B(:,:,1) = eye(D);
    y_guess(:,1) = states(i-1,:) + h*diffeq(states(i-1,:),inputs(i-1,:),t(i-1))';
    f_guess(:,1) = get_f(states(i-1,:)', y_guess(:,1), inputs(i-1,:), inputs(i,:), t(i-1), t(i))';
    y_guess(:,2) = y_guess(:,1) - B(:,:,1)*f_guess(:,1);
    for k = 2:iterations-1
      f_guess(:,k) = get_f(states(i-1,:)', y_guess(:,k), inputs(i-1,:), inputs(i,:), t(i-1), t(i))';
